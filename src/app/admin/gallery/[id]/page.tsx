	...
	return (
		<div className="pb-12">
			...
			<div className="space-y-6">
				...
				<section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><h2 className="text-lg font-bold text-gray-900">Album images</h2><p className="mt-1 text-sm text-gray-500">Upload one or many images, reorder them, choose the cover, and add bilingual details.</p></div><span className="rounded-full bg-[#01334D]/5 px-3 py-1 text-xs font-bold text-[#01334D]">{album.images.length} / {GALLERY_MAX_IMAGES} images</span></div><div className="mt-6"><GalleryImagesEditor images={album.images} coverImageUrl={album.coverImageUrl} onChange={(images) => setAlbum((current) => ({ ...current, images }))} onCoverChange={(coverImageUrl) => setAlbum((current) => ({ ...current, coverImageUrl }))} /></div></section>
			</div>
		</div>
	);
}
